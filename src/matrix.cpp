...
/* Multiply 4x4 matrix m1 by 4x4 matrix m2 and store the result in res */
void multiply4x4n(GLfloat *res, GLfloat *m1, GLfloat *m2)
{
    GLfloat temp[16];
    ...
    memcpy(res, temp, sizeof(GLfloat) * 16);
}
...
/* Rotate a matrix by an angle on a X, Y, or Z axis */
void rotate_r(GLfloat *matrix, GLfloat angle, AXIS axis)
{
    const GLfloat d2r = 0.0174532925199; /* PI / 180 */
    const int cos1[3] = { 5, 0, 0 };
    const int cos2[3] = { 10, 10, 5 };
...
//    multiply4x4(matrix, newmatrix);
    multiply4x4(newmatrix, matrix);
    memcpy(matrix, newmatrix, sizeof(GLfloat) * 16);
}

/* Scale a matrix by an X, Y, or Z axis */
...
/* Scale a matrix by an X, Y, or Z axis */
void scale_r(GLfloat *matrix, GLfloat x, GLfloat y, GLfloat z)
{
    GLfloat newmatrix[16] = IDENTITY_MATRIX4;
